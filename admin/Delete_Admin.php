...
if(isset($_POST['delete'])){
  ...
  $Num_Registro_Mascota = $_REQUEST['id_user'];
  $password = $_POST['password'];
  
  if($contraseña === $_POST['password']){
  /* AQUI REALIZAMOS EL UPDATE DEL REGISTRO QUE SE SELECCIONÓ */
  $query3="DELETE FROM usuarios WHERE id_user = $id_user";
  $ResultadoDeleteAdmin = $con->query($query3);

  if($ResultadoDeleteAdmin){
  echo "<script>alert('Los datos se han eliminado correctamente');window.location='Config.php?modulo=ViewAdmin'</script>";
  }else{
